...
## Historiador
This program implements a UDP-based historian system for recording and managing events received from multiple remote units (UTRs). It is designed to run on a Linux environment using sockets, semaphores, and multithreading. The main goal is to receive, store, and optionally display incoming data from UTR devices, while also providing interactive control through a user menu.

### 🔧 Key Features

- UDP Communication:
  The program sets up a UDP socket to listen for incoming messages from UTR devices.
  Each message is received, stored in a two-dimensional array (StringArray), and optionally displayed on the terminal.
  A response message is sent back to confirm reception.
- Data Storage (Historian)
  Incoming data packets are stored sequentially in memory (up to 1000 records).
  The program can display this data in real time or later upon user request.
  Data is also written into a text file (DataBaseUTRs.txt) for persistence.

- Multithreading
  Communication Thread (hilo_comunicacion): Handles continuous UDP reception, storage, and feedback to UTR devices.
  Menu Thread (hilo_menu): Provides an interactive console-based menu to the user for controlling LEDs on UTR devices, enabling/disabling continuous display, or printing the full history.

- Semaphore Synchronization:
  A semaphore ensures safe access to shared memory (StringArray) between the communication and menu threads.

- IoT Device Control
  Through menu commands, the user can send specific control codes to UTRs to turn LEDs ON/OFF individually or all at once.
  Commands are transmitted via UDP, and corresponding confirmations are printed.

### 🖥️ User Interaction

The user interacts through a simple menu-driven interface in the console:

 - con → Enables continuous display of received data.
 - uon → Turns on LEDs in the UTRs.
 - off → Turns off LEDs in the UTRs.
 - ver → Displays the full history of stored events.

### ⚙️ Technical Aspects
- Uses POSIX threads (pthread) for concurrency.
- Relies on UDP sockets for lightweight communication.
- Implements semaphores to avoid race conditions.
- Includes error handling for socket operations.
- Logs data with timestamps into an external file for traceability.
...
===================================================================== */
#define MSG_SIZE 60		// Maximum message size
#define INIT_VALUE	1
#define DataBaseUTRs "DataBaseUTRs.txt"
/* =====================================================================
	Function prototypes
===================================================================== */
void hilo_comunicacion(void *ptr);	// for communication thread
void hilo_menu(void *ptr);			// for IoT event thread
...
char buffer[100];
// for received data
char StringArray[1000][100];		// able to log 1 hour with 2 UTRs
...
/* =====================================================================
	main, 2nd argument is the port >1024
===================================================================== */
int main(int argc, char *argv[])
{
	...
	server.sin_family = AF_INET;		// symbolic constant for Internet domain
	server.sin_addr.s_addr = htonl(INADDR_ANY);	// to receive from any network interface
	...
	//---------------------- creation and joining of odd/even lines
	printf("Threads ended\n");
	return 0; 		// close socket when no longer in use
	exit(0);
}
/* =====================================================================
	Functions 
...
void hilo_comunicacion(void *ptr)
{
	...
	while(1)
	{
		//---------------------- reception of values received from UTRs
		n = recvfrom(sockfd, buffer, 50, 0, (struct sockaddr *)&from, &length);
	    if(n < 0)
	 		error("recvfrom received"); 
		//---------------------- save received values in database
		sem_wait(&my_semaphore);
		memcpy(StringArray[contador],buffer,strlen(buffer));
		sem_post(&my_semaphore);
		
		//---------------------- check if continuous display is enabled from menu
		if (continuo==0)
		{
			contador++;
		}
		...
		{
			puts("U E Time                   Sw Bt Led V \n");
			printf("%s\n",StringArray[contador]);
			contador++;
			fflush(stdout);
		}
		//---------------------- confirmation message of reception
...
/* ==========
	Function for IoT event thread
=============*/
void hilo_menu(void *ptr)
{
	FILE *datos = fopen(DataBaseUTRs, "w");
	fputs("U E Time   Sw Bot LED V\n", datos);
	/* =======================================================================
		IoT event loop
	========================================================================= */
	while(1)
	{
		...
		printf("--------------------------------------\n");
		printf("--------------------------------------\n");
		printf("  WELCOME TO THE HISTORIAN!\n");
		printf("  Enter the menu you want to display\n");
		printf("  Continuous display: con\n");
		printf("  Turn on LEDs in UTRs: uon\n");
		printf("  Turn off LEDs in UTRs: off\n");
		printf("  Show complete history: ver\n");
		printf("--------------------------------------\n");
		printf("--------------------------------------\n");
		scanf("%9s", input);
		...
		{
			continuo=1;
			printf("  CONTINUOUS DISPLAY\n");
			puts("U E Time   Sw Bt Led V n");
		}
		else 
			continuo=0;
		//---------------------- LED control in UTRs
		if (strcmp(input, "uon")==0)	
		{
			continuo=0;
			printf("--------------------------------------\n");
			printf("  Select LEDs to turn on\n");	
			printf("  UTR1 LED1: L11\n");
			printf("  UTR1 LED2: L12\n");
			printf("  UTR2 LED1: L21\n");
			...
			{
				sprintf(mensaje,"111\n");
				printf("You turned on Led1 UTR1\n");
			}
			else if (strcmp(input, "L12")==0)
			{
				sprintf(mensaje,"112\n");
				printf("You turned on Led2 UTR1\n");
			}
			else if (strcmp(input, "L21")==0)
			{
				sprintf(mensaje,"121\n");
				printf("You turned on Led1 UTR2\n");
			}
			else if (strcmp(input, "L22")==0)
			{
				sprintf(mensaje,"122\n");
				printf("You turned on Led2 UTR2\n");
			}
			else if (strcmp(input, "all")==0)
			{
				sprintf(mensaje,"all\n");
				printf("You turned on all LEDs\n");
			}
		}
		//---------------------- turn off UTRs
		if (strcmp(input, "off")==0)			
		{
			continuo=0;
			printf("--------------------------------------\n");
			printf("Select LEDs to turn off\n");	
			printf("  UTR1 LED1: L11\n");
			printf("  UTR1 LED2: L12\n");
			printf("  UTR2 LED1: L21\n");
			printf("  UTR2 LED2: L22\n");
			...
			{
				sprintf(mensaje,"011\n");
				printf("You turned off Led1 UTR1\n");
			}
			else if (strcmp(input, "L12")==0)
			{
				sprintf(mensaje,"012\n");
				printf("You turned off Led2 UTR1\n");
			}
			else if (strcmp(input, "L21")==0)
			{
				sprintf(mensaje,"021\n");
				printf("You turned off Led1 UTR2\n");
			}
			else if (strcmp(input, "L22")==0)
			{
				sprintf(mensaje,"022\n");
				printf("You turned off Led2 UTR2\n");
			}
			if (strcmp(input, "all")==0)
			{
				sprintf(mensaje,"000\n");
				printf("You turned off all LEDs\n");
			}
		}
		//---------------------- display stored data
...
{
    perror(msg);	// writes to standard error (the terminal, by default)
    exit(1);
}
